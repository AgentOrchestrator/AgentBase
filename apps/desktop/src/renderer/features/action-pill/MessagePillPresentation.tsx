/**
 * MessagePill Presentation Component
 *
 * Pure UI component that renders the message input pill.
 * All state and business logic comes from props.
 */

import './MessagePill.css';

export interface MessagePillPresentationProps {
  // Data
  inputValue: string;
  isSending: boolean;
...
export function MessagePillPresentation({
  inputValue,
  isSending,
  targetAgentId,
  canSend,
  ...
  onSend,
  onKeyDown,
}: MessagePillPresentationProps) {
  return (
    <div
      className={`message-pill ${targetAgentId ? 'has-target' : ''} ${isSending ? 'sending' : ''}`}
    >
      <div className="message-pill-input-area">
        {targetAgentId && (
          <div className="message-pill-target-indicator" title={`Sending to: ${targetAgentId}`}>
            <span className="message-pill-target-dot" />
          </div>
        )}
        <textarea
          ...
          onKeyDown={onKeyDown}
          placeholder={targetAgentId ? `Message to agent...` : 'Ask anything...'}
          rows={1}
          disabled={isSending}
        />
        <button
          className="message-pill-mic-button"
          type="button"
          aria-label="Voice input"
          disabled={isSending}
        >
          <svg
            className="message-pill-mic-icon"
            width="16"
            height="16"
            viewBox="0 0 140 223"
            fill="none"
        ...
        >
          {isSending ? (
            <span className="message-pill-send-spinner" />
          ) : (
