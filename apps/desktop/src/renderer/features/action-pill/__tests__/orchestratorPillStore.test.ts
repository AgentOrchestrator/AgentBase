 ...
 *
 * Tests the orchestrator UI store with mocked API.
 */

import { beforeEach, describe, expect, it, vi } from 'vitest';
import { selectCanSend, useOrchestratorPillStore } from '../store/orchestratorPillStore';
import type { OrchestratorAPI } from '../store/orchestratorPillTypes';
...
describe('OrchestratorPillStore', () => {
  ...
  describe('Initialization', () => {
    it('checks CLI health on initialize', async () => {
      vi.mocked(mockOrchestratorAPI.getHealth).mockResolvedValue({
        cliAvailable: true,
        lastHealthCheck: Date.now(),
      });
      vi.mocked(mockOrchestratorAPI.getMostRecentConversation).mockResolvedValue(null);
    ...
    it('restores most recent conversation on initialize', async () => {
      ...
      vi.mocked(mockOrchestratorAPI.getMostRecentConversation).mockResolvedValue({
        id: 'conv-123',
        createdAt: 1000,
        updatedAt: 2000,
      });
      vi.mocked(mockOrchestratorAPI.getMessages).mockResolvedValue([
  ...
  describe('Input Management', () => {
    ...
    it('clears input after send', async () => {
      vi.mocked(mockOrchestratorAPI.createConversation).mockResolvedValue({
        id: 'conv-1',
        createdAt: 1000,
        updatedAt: 1000,
      });
      vi.mocked(mockOrchestratorAPI.sendMessage).mockImplementation(async () => ({
        content: 'Response',
        toolCalls: [],
      }));
  ...
  describe('Sending Messages', () => {
    it('creates conversation if none exists', async () => {
      ...
      vi.mocked(mockOrchestratorAPI.sendMessage).mockResolvedValue({
        content: 'Response',
        toolCalls: [],
      });
    ...
    it('streams response and updates streaming content', async () => {
      vi.mocked(mockOrchestratorAPI.sendMessage).mockImplementation(
        async (_convId, _msg, onChunk) => {
          onChunk('Hello ');
          onChunk('world!');
    ...
    it('sets isSending during request', async () => {
      ...
      vi.mocked(mockOrchestratorAPI.sendMessage).mockImplementation(
        () =>
          new Promise((resolve) => {
            resolveSend = () => resolve({ content: 'Done', toolCalls: [] });
          })
      );
  ...
  describe('Reset', () => {
    it('clears all state on reset', () => {
      useOrchestratorPillStore.setState({
        ...
        messages: [
          { id: 'm1', conversationId: 'conv-1', role: 'user', content: 'Hello', timestamp: 1000 },
        ],
        inputValue: 'Draft',
