...
const representation_1 = require("./services/representation");
// Map to store terminal instances by ID
const terminalProcesses = new Map();
// Database instance
let database;
// RepresentationService instance and dependencies
const representationLogger = {
    info: (message, context) => console.log(`[Representation] ${message}`, context || ''),
    warn: (message, context) => console.warn(`[Representation] ${message}`, context || ''),
    error: (message, context) => console.error(`[Representation] ${message}`, context || ''),
};
const representationIdGenerator = {
    generate: () => crypto.randomUUID(),
};
const representationService = new representation_1.RepresentationService({ defaultTimeout: 30000, initializeProvidersOnStart: true }, { logger: representationLogger, idGenerator: representationIdGenerator });
const createWindow = () => {
...
// ============================================
electron_1.ipcMain.handle('representation:get-available-types', async () => {
    try {
...
electron_1.ipcMain.handle('representation:transform', async (_event, providerId, input) => {
    try {
...
electron_1.ipcMain.handle('representation:transform-to-image', async (_event, input, options) => {
    try {
...
electron_1.ipcMain.handle('representation:transform-to-summary', async (_event, input, options) => {
    try {
...
electron_1.ipcMain.handle('representation:transform-to-audio', async (_event, input, options) => {
    try {
...
electron_1.ipcMain.handle('representation:get-all-providers', async () => {
    try {
...
async function commandExists(command) {
    return new Promise((resolve) => {
        const which = (0, child_process_1.spawn)('which', [command]);
...
electron_1.ipcMain.handle('shell:open-with-editor', async (_event, directoryPath, editor) => {
    try {
        ...
        if (editor === 'finder') {
            electron_1.shell.openPath(directoryPath);
            return { success: true };
        }
        ...
        if (config.command) {
            ...
            if (exists) {
                (0, child_process_1.spawn)(config.command, config.args(directoryPath), { detached: true, stdio: 'ignore' }).unref();
                return { success: true };
            }
        }
        ...
        if (config.app) {
            ...
            if (exists) {
                (0, child_process_1.spawn)('open', ['-a', config.app, directoryPath], { detached: true, stdio: 'ignore' }).unref();
                return { success: true };
            }
        }
...
electron_1.ipcMain.handle('shell:get-available-editors', async () => {
    try {
        const available = [];
        for (const [editor, config] of Object.entries(EDITOR_COMMANDS)) {
            // Check command
            if (config.command) {
                const exists = await commandExists(config.command);
                if (exists) {
...
electron_1.ipcMain.handle('shell:show-in-folder', async (_event, filePath) => {
    try {
