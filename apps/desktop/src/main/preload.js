...
electron_1.contextBridge.exposeInMainWorld('representationAPI', {
    getAvailableTypes: () => unwrapResponse(electron_1.ipcRenderer.invoke('representation:get-available-types')),
    transform: (providerId, input) => unwrapResponse(electron_1.ipcRenderer.invoke('representation:transform', providerId, input)),
    transformToImage: (input, options) => unwrapResponse(electron_1.ipcRenderer.invoke('representation:transform-to-image', input, options)),
    transformToSummary: (input, options) => unwrapResponse(electron_1.ipcRenderer.invoke('representation:transform-to-summary', input, options)),
    transformToAudio: (input, options) => unwrapResponse(electron_1.ipcRenderer.invoke('representation:transform-to-audio', input, options)),
    getAllProviders: () => unwrapResponse(electron_1.ipcRenderer.invoke('representation:get-all-providers')),
});
// Expose shell API
electron_1.contextBridge.exposeInMainWorld('shellAPI', {
    openWithEditor: async (directoryPath, editor) => {
        await unwrapResponse(electron_1.ipcRenderer.invoke('shell:open-with-editor', directoryPath, editor));
    },
    getAvailableEditors: () => unwrapResponse(electron_1.ipcRenderer.invoke('shell:get-available-editors')),
    showInFolder: async (path) => {
        await unwrapResponse(electron_1.ipcRenderer.invoke('shell:show-in-folder', path));
    },
});
